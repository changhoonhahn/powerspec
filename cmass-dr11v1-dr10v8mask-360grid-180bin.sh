...
for P0 in 20000
do
    fname=$datadir"cmass-dr11v1-S-Anderson-nzw-zlim-dr10v8mask.dat"
    FFTname=$FFTdir$FFT"cmass-dr11v1-S-Anderson-nzw-zlim-dr10v8mask"$fftext
    echo $fname
    echo $FFTname
    ./FFT-fkp-nzw-ngalsys-360grid.exe $Rbox 0 $P0 $fname $FFTname

    randfname=$datadir"cmass-dr11v1-S-Anderson-nzw-zlim-dr10v8mask.ran.dat"
    FFTrandname=$FFTdir$FFT"cmass-dr1v1-S-Anderson-nzw-zlim-dr10v8mask"$fftranext
    if [ -a $FFTrandname ] 
    then
    ...
    powername=$powerdir$power"cmass-dr11v1-S-Anderson-nzw-zlim-dr10v8mask"$pwrext
