...
!         close(7)
         ...
         OPEN(UNIT=4,FILE=dcgfname,STATUS='unknown',FORM='formatted')
          DO iz=1,Ngrid
             DO iy=1,Ngrid
                 DO ix=1,Ngrid/2+1
                     WRITE(4,1017) ix,iy,iz,real(dcg(ix,iy,iz))
     &              ,aimag(dcg(ix,iy,iz))
                 ENDDO
             ENDDO
          ENDDO
          CLOSE(4)
 1017     FORMAT(3(2x,I3),2x,E16.6,2x,E16.6)
 ...
 15      continue
         ...
         read(*,'(a)') dcrfile
         dcgfname='/mount/chichipio2/hahn/FFT/'//dcrfile
         OPEN(UNIT=4,FILE=dcgfname,STATUS='unknown',FORM='formatted')
          DO iz=1,Ngrid
             DO iy=1,Ngrid
                 DO ix=1,Ngrid/2+1
                     WRITE(4,1015) ix,iy,iz,real(dcr(ix,iy,iz))
     &              ,aimag(dcr(ix,iy,iz))
                 ENDDO
             ENDDO
          ENDDO
