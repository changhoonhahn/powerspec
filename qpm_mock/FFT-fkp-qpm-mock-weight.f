      ...
      integer Ng,Nr,iflag,ic,Nbin,l,ipoly
      integer*8 planf
      ...
      real cz,sec2(Nsel),chi,nbar,Rbox,wsys,wqpm,rwqpm
      real*8 rwsys 
...
!      write(*,*)'FKP weight P0?'
 ...
 13      continue
       ...
       elseif (iflag.eq.1) then ! compute discretness integrals and FFT random mock
         ...
         do i=1,Nmax
            read(4,*,end=15)ra,dec,az,rwqpm
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
            wr(i)=rwqpm
            Nran=Nran+1
!            WRITE(*,*) wr(i),rwqpm,rwsys,Nran
 ...
 15      continue
         ...
         rwsys=0.d0
         DO 100 i=1,Nran
            rwsys=rwsys+DBLE(wr(i))
 100     CONTINUE

         WRITE(*,*) 'Wr,sys=',rwsys,'Nr,sys=',rwsys/float(Nran)
