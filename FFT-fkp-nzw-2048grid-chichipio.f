      ...
      integer*8 Nsel,Nran,i,iwr,Ngal,Nmax,n,kx,ky,kz,Lm,Ngrid
      integer*8 ix,iy,iz,j,k
      integer*8 Ng,Nr,iflag,ic,Nbin, l
      integer*8 planf
      real pi,cspeed
      parameter(Nsel=201,Nmax=2*10**8,Ngrid=2048,Nbin=151)
      parameter(pi=3.141592654)
      integer*8 grid
      dimension grid(3)
      parameter(cspeed=299800.0)
      integer*8, allocatable :: ig(:),ir(:)
...
cc*******************************************************************
      real*8 dtl(2*L,L,L),r(3,N),rm(2),we,ar,nb,nbg(N),w(N)
      integer*8 ig(N)
      external nbar
...
c^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      integer*8 neval,ier,iord(limit),last
...
cc*******************************************************************
      implicit none
      integer*8 L,Lnyq,ix,iy,iz,icx,icy,icz,N
