...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
 ...
 13      continue
        ...
        call assign_CIC(Ngal,rg,rm,Lm,dcg,wg)
         ...
         open(unit=7,file=outname,status='unknown',form='unformatted')
         write(7)(((dcg(ix,iy,iz),ix=1,Lm/2+1),iy=1,Lm),iz=1,Lm)
         write(7)P0,Ng,wsys
         close(7)
...
2          dtl(ix,iy,iz)= 0.0
      ...
      do i=1,Npar
         ...
         dtl(ixp,iyp,izp)=dtl(ixp,iyp,izp)+(1.-dx)*(1.-dy)*(1.-dz)*wg(i)
      enddo
