...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
     ...
     $     FFTW_ESTIMATE + FFTW_IN_PLACE)
      ...
      write(*,*)'file with number density array of catalog'
      read(*,'(a)')selfunfile
      open(unit=3,file=selfunfile,status='old',form='formatted')
      do l=1,Nsel
               read(3,*,end=12) zt, numden
c               zt = zmax*float(l-1)/float(Nsel-1)
               z(l)= zt
               selfun(l)=numden
      enddo
 12   continue
...
!         write(*,*)'Name of spline/splint test file'
!         read(*,'(a)')spltest
!         open(unit=7,file=spltest,action='write',status='replace') 
         do i=1,Nmax
c            read(4,*,end=13)rg(1,i),rg(2,i),rg(3,i),nbg(i) 
            read(4,*,end=13)ra,dec,az,w,barn
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
...
!         close(7)
       ...
       elseif (iflag.eq.1) then ! compute discretness integrals and FFT random mock
         ...
         do i=1,Nmax
            read(4,*,end=15)ra,dec,az,w,barn
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
...
c      parameter(Nmax=2*10**7)
     ...
     $           abs(rg(3,i)).lt.Rbox) then !put into box
      ...
      END
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      subroutine assign(N,r,rm,L,dtl,P0,nbg,ig,wg) !with FKP weighing
cc*******************************************************************
      real dtl(2*L,L,L),r(3,N),rm(2),we,ar,nb,nbg(N),wg(N)
      integer ig(N)
      external nbar
