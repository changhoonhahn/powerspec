...
bispec="BISPcmass_dr10_north_ir4"
...
ifort -O4 -openmp -o bisp_para_maneramock_v5p2.exe bisp_para_maneramock_v5p2.F90 -L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw
...
do
    ...
    do 
        ...
        then 
            ...
            bispec=$BISdir$bispec"00"$i$suffix".grid"$grid".P0"$P0".box"$box
        elif [ $i -lt 100 ] 
        then
            FFTname=$FFTdir$prefix"0"$i$suffix".grid"$grid".P0"$P0".box"$box
            bispec=$BISdir$bispec"0"$i$suffix".grid"$grid".P0"$P0".box"$box
        else 
            FFTname=$FFTdir$prefix$i$suffix".grid"$grid".P0"$P0".box"$box
            bispec=$BISdir$bispec$i$suffix".grid"$grid".P0"$P0".box"$box
        fi
        echo $FFTname
        echo $randFFTname 
        echo $bispec

        ./bisp_para_maneramock_v5p2.exe $iflag $randFFTname $FFTname $bispec
    done
done
