...
prefix="FFTcmass_dr11_north_ir4"
bispecpre="BISPcmass_dr11_north_ir4"
suffix=".v7.0.wghtv.txt"
sscale=3600.0
box="3600"
grid="360"
nmax="40"
...
BISdir="/mount/riachuelo1/hahn/bispec/manera_mock/dr11/"

ifort -O4 -o bisp_maneramock_v5p2.exe bisp_maneramock_v5p2.f -L/usr/local/fftw_intel_s/lib -lsrfftw -lsfftw

for P0 in 20000
do
    randFFTname=$FFTdir"FFTcmass_dr11_north_randoms_ir4_combined_wboss.v7.0.wghtv.txt.grid"$grid".P0"$P0".box"$box
    for i in {1..100}
    do 
        if [ $i -lt 10 ]
        then 
