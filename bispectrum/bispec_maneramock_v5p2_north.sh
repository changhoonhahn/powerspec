...
do
    ...
    do 
        ...
        then 
            ...
            bispec=$BISdir$bispec"00"$i$suffix".grid"$grid".nmax"$nmax".nstep"$nstep".P0"$P0".box"$box
        elif [ $i -lt 100 ] 
        then
            FFTname=$FFTdir$prefix"0"$i$suffix".grid"$grid".P0"$P0".box"$box
            bispec=$BISdir$bispec"0"$i$suffix".grid"$grid".nmax"$nmax".nstep"$nstep".P0"$P0".box"$box
        else 
            FFTname=$FFTdir$prefix$i$suffix".grid"$grid".P0"$P0".box"$box
            bispec=$BISdir$bispec$i$suffix".grid"$grid".nmax"$nmax".nstep"$nstep".P0"$P0".box"$box
        fi
        echo $FFTname
        echo $randFFTname 
        echo $bispec
