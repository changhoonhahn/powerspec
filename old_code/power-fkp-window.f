...
c      complex dcg(Ngrid/2+1,Ngrid,Ngrid),dcr(Ngrid/2+1,Ngrid,Ngrid)
 ...
 100  continue
      ...
      do 105 iz=1,Ngrid
        ...
        do 105 iy=1,Ngrid
            iky=mod(iy+Ngrid/2-2,Ngrid)-Ngrid/2+1
            do 105 ix=1,Ngrid/2+1
                rki=sqrt(real((ix-1)**2+iky**2+ikz**2))
                imk=nint(Nbins*rki/nyq)
                if(imk.le.Nbins .and. imk.ne.0)then

                    do 115 jz=1,Ngrid
                        jkz=mod(jz+Ngrid/2-2,Ngrid)-Ngrid/2+1
                        do 115 jy=1,Ngrid
                            jky=mod(jy+Ngrid/2-2,Ngrid)-Ngrid/2+1
                            do 115 jx=1,Ngrid/2+1
                                rkj=sqrt(real((jx-1)**2+jky**2+jkz**2))
                                rkij=sqrt(real((ix-jx)**2+(iky-jky)**2+
                               &    (ikz-jkz)**2))
                                jmk=nint(Nbins*rkj/nyq)
                                ijmk=nint(Nbins*rkij/nyq)
                                if(jmk.le.Nbins .and. jmk.ne.0 .and.
                               &    ijmk.le.Nbins .and. ijmk.ne.0 )then
                                    avgPw(imk*Nbin+jmk)=avgPr(ijmk)/
                                   &    co(ijmk)
                                   
                                    avgPt(jmk)=avgPr(imk)/avgPw
