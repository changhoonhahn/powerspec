...
c      complex dcg(Ngrid/2+1,Ngrid,Ngrid),dcr(Ngrid/2+1,Ngrid,Ngrid)
      ...
      character fname*200,fnamer*200,powername*200
      ...
      fname='/global/data/scr/chh327/powercode/FFT/'//filecoef
      open(unit=4,file=fname,status='old',form='unformatted')
      read(4)dcg
      read(4)P0m,Ng 
      close(4)
      fnamer='/global/data/scr/chh327/powercode/FFT/'//filecoefr
      open(unit=4,file=fnamer,status='old',form='unformatted')
      read(4)dcr
