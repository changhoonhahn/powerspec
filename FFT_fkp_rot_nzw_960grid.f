      ...
      real gx,gy,gz,rx,ry,rz,w,nbb
      !For CMASS-DR10V5-N-ANDERSON
      real rtheta, rphi
!      parameter(rtheta=1.9064785,rphi=2.9532971)

      !For CMASS-DR10V5-S-ANDERSON
      parameter(rtheta=0.17987521,rphi=3.0346565)
...
c      complex dcg(Ngrid,Ngrid,Ngrid),dcr(Ngrid,Ngrid,Ngrid)
      ...
      character spltest*200,path*200,fname*200,outname*200
     ...
     $     FFTW_ESTIMATE + FFTW_IN_PLACE)
      ...
      if (iflag.eq.0) then ! run on mock
         ...
         read(*,'(a)')lssfile
         fname='/global/data/scr/chh327/powercode/data/'//lssfile
         write(*,*)fname
         ...
         do i=1,Nmax
            ...
            dec=dec*(pi/180.)
            rad=chi(az)
            gx=rad*cos(dec)*cos(ra)
            gy=rad*cos(dec)*sin(ra)
            gz=rad*sin(dec)
            !Completeness Weight of the galaxy
            wg(i)=w
            rg(1,i)=cos(rtheta)*cos(rphi)*gx+sin(rtheta)*cos(rphi)*gy-
     &       sin(rphi)*gz
            rg(2,i)=cos(rtheta)*gy-sin(rtheta)*gx
            rg(3,i)=cos(rtheta)*sin(rphi)*gx+sin(rtheta)*sin(rphi)*gy+
     &       cos(rphi)*gz
            nbg(i)=nbb
            Ngal=Ngal+1
         enddo
...
!         close(7)
       ...
       elseif (iflag.eq.1) then ! compute discretness integrals and FFT random mock
         ...
         read(*,'(a)')randomfile
         fname='/global/data/scr/chh327/powercode/data/'//randomfile
         ...
         do i=1,Nmax
            ...
            dec=dec*(pi/180.)
            rad=chi(az)
            rx=rad*cos(dec)*cos(ra)
            ry=rad*cos(dec)*sin(ra)
            rz=rad*sin(dec)
            rr(1,i)=cos(rtheta)*cos(rphi)*rx+sin(rtheta)*cos(rphi)*ry-
     &       sin(rphi)*rz
            rr(2,i)=cos(rtheta)*ry-sin(rtheta)*rx
            rr(3,i)=cos(rtheta)*sin(rphi)*rx+sin(rtheta)*sin(rphi)*ry+
     &       cos(rphi)*rz
            nbr(i)=nbb
            wr(i) = w
            Nran=Nran+1
