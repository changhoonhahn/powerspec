...
ifort -O3 -o FFT-fkp-mock-dr11-v7p0-upweight_360grid.exe FFT-fkp-mock-dr11-v7p0-upweight_360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
then
    ...
    ./FFT-fkp-mock-dr11-v7p0-upweight_360grid.exe $Rbox 1 $P0 $datadir$nbarfname $datadir$randname $FFTrandname
fi
...
do
    ...
    then
        ...
        FFTname=$FFTdir$FFT$name0"00"$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"00"$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
    elif [ $i -lt 100 ]
    then
        fname=$datadir$name0"0"$i$nameend
        FFTname=$FFTdir$FFT$name0"0"$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"0"$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
    else
        fname=$datadir$name0$i$nameend
        FFTname=$FFTdir$FFT$name0$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0$i$nameend"_upweighted.grid"$grid".P0"$P0".box"$box
    fi
    echo $fname
    echo $FFTname
    echo $powername
    ./FFT-fkp-mock-dr11-v7p0-upweight_360grid.exe $Rbox 0 $P0 $datadir$nbarfname $fname $FFTname
