pro fibcoll_pthalo_dlos_shuffle_zlim, n, north=north, south=south
...
;If w_upcp matches the number of neighbors then calculate d_LOS:
    ...
    for ii=0L,n_elements(gal_upcp)-1L do begin 
        ...
        if w_cp[targ_indx] eq n_elements(neigh_indx)+1L then begin
            ...
            for k=0L,n_elements(neigh_indx)-1L do begin
                d_los = disp_los[dloscount] 
                neigh_r[k] = targ_r+d_los
                if neigh_r[k] ge comdis_high then begin 
                    high_out_count = high_out_count+1
                    neigh_r[k] = targ_r-d_los
                endif 
                if neigh_r[k] le comdis_low then begin
                    low_out_count = low_out_count+1 
                    neigh_r[k] = targ_r-d_los
                endif 
    ...
    print,'Number of galaxies within redshift limits=',n_elements(dm[where(dm lt comdis_high and dm gt comdis_low)])
    print,'Number of galaxies above redshift limit=',high_out_count
    print,'Number of galaxies below redshift limit=',low_out_count
    print,'total(w_cp)=',total(w_cp)

    outfname='shuffle_zlim_cmass_dr11_'+NS+'_ir4'+strmid(strtrim(string(n+1000),1),1)+'.v7.0.wghtv.txt'
