...
dlosfname="pthalo-ir4211-dr11-v7p0-N-disp_los_hist_normed.dat"
P0=20000
sscale=3600.0
Rbox=1800.0
box="3600"
...
ifort -O3 -o FFT-fkp-mock-cp-dloshist-360grid.exe FFT-fkp-mock-cp-dloshist-360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
do
    ...
    then
        ...
        FFTname=$FFTdir$FFT$name0"00"$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"00"$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
    elif [ $i -lt 100 ]
    then
        fname=$datadir$name0"0"$i$nameend
        FFTname=$FFTdir$FFT$name0"0"$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"0"$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
    else
        fname=$datadir$name0$i$nameend
        FFTname=$FFTdir$FFT$name0$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0$i$nameend"-cp-pthalo-dloshist.grid"$grid".P0"$P0".box"$box
    fi
    echo $fname
    echo $FFTname
    echo $powername

    ./FFT-fkp-mock-cp-dloshist-360grid.exe $Rbox 0 $P0 $datadir$nbarfname $prismdir$dlosfname $fname $FFTname
