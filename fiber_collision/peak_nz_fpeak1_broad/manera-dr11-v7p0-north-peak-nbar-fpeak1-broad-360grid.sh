...
nameend=".v7.0.wghtv.txt"; nameend_peak=".v7.0.peaknz.fpeak1.broad.txt"
...
echo "n="; read n 
idl -e ".r fibcoll_make_rand_peaknz_fpeak1_broad.pro"
idl -e ".r fibcoll_nbar_comp_norm.pro"
ifort -O3 -o mock-cp-dlos-peaknz-fpeak1-broad.exe mock-cp-dlos-peaknz-fpeak1-broad.f
ifort -O3 -o FFT-fkp-mock-peaknz-fpeak1-broad-360grid.exe FFT-fkp-mock-peaknz-fpeak1-broad-360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
#    if [[ ! -a $corrfile ]]; then 
    ./mock-cp-dlos-peaknz-fpeak1-broad.exe $datadir$nbarfname$dat $datadir$tailnbarfname $fname $corrfile
    echo $corrfile
#    fi
done
wait
...
for i in $(seq 1 $n); do
    ...
    if [[ ! -a $corrrandfile ]]; then   
        idl -e "fibcoll_make_rand_peaknz_fpeak1_broad,"$i",466264987" & 
        echo $corrrandfile
    fi
    jobcnt=(`jobs -p`)
    if [ ${#jobcnt[@]} -gt 3 ]; then
...
if [[ ! -a $datadir$randcomb_fname ]]; then 
    ipython -noconfirm_exit rand_combine-peaknz_fpeak1_broad.py $n $randcomb_fname
fi 

# n(z) correction for FKP weights:
ipython -noconfirm_exit fibcoll_nbar_wfkp_peak_nbar.py $n $datadir$nbarfname
nbar_wfkp_corr_fname=$nbarfname"-peaknz_fpeak1_broad.dat"

FFTrandname=$FFTdir$FFT"cmass_dr11_north_"$n"_randoms_ir4_combined"${nameend_peak:0:$((${#nameend_peak}-4))}".grid"$grid".P0"$P0".box"$box
./FFT-fkp-mock-peaknz-fpeak1-broad-360grid.exe $Rbox 1 $P0 $datadir$nbar_wfkp_corr_fname $datadir$tailnbarfname $datadir$randcomb_fname $FFTrandname
echo $FFTrandname
...
for i in $(seq 1 $n); do 
    ...
    powername=$powerdir$power$name0$i".v7.0."$n"randoms.peaknz.fpeak1.broad.grid"$grid".P0"$P0".box"$box
     
    ./FFT-fkp-mock-peaknz-fpeak1-broad-360grid.exe $Rbox 0 $P0 $datadir$nbar_wfkp_corr_fname $datadir$tailnbarfname $fname $FFTname
    echo $FFTname
