...
disp_los = np.loadtxt(prismdir+'cmass-dr11v2-N-Anderson-disp_los.dat')
...
popt, pcov = curve_fit(gauss, mpc_bin[7000:13000], hist_disp_los[0][7000:13000])
print popt
...
np.savetxt(prismdir+'cmass-dr11v2-N-Anderson-disp_los_hist_normed.dat', output)


for d in [20, 30, 40]:
    RMSfrac = float(len(disp_los[(disp_los<d) & (disp_los>-d)]))/float(len(disp_los))*100.0
    caption = r''+str(np.int(RMSfrac))+"$\%$"
    ax1.annotate(caption, (float(d),20), xycoords='data', xytext=(float(d), 50), textcoords='data',
