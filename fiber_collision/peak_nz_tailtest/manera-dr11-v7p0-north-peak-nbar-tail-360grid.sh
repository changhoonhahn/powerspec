...
nameend=".v7.0.wghtv.txt"; nameend_peak=".v7.0.peakcorrtail.txt"
...
echo "n="; read n 
idl -e ".r fibcoll_make_rand_peak_nbar_tail.pro"
idl -e ".r fibcoll_nbar_comp_norm.pro"
ifort -O3 -o mock-cp-dlos-peak-nbar-tail.exe mock-cp-dlos-peak-nbar-tail.f
...
#    if [[ ! -a $corrfile ]]; then 
    ./mock-cp-dlos-peak-nbar-tail.exe $datadir$nbarfname$dat $datadir$tailnbarfname $fname $corrfile &
    echo $corrfile
#    fi
    jobcnt=(`jobs -p`)
    if [ ${#jobcnt[@]} -gt 3 ]; then
...
#    if [[ ! -a $corrrandfile ]]; then   
    idl -e "fibcoll_make_rand_peak_nbar_tail,"$i",466264987" & 
    echo $corrrandfile
#    fi
    jobcnt=(`jobs -p`)
    if [ ${#jobcnt[@]} -gt 3 ]; then
...
nbar_wfkp_corr_fname=$nbarfname"-peaknbarcorrtail.dat"
...
for i in $(seq 1 $n); do 
    ...
    FFTname=$FFTdir$FFT$name0$i".v7.0.peakcorrtail.grid"$grid".P0"$P0".box"$box
    powername=$powerdir$power$name0$i".v7.0."$n"randoms.peakcorrtail.grid"$grid".P0"$P0".box"$box
