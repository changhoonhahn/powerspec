...
ifort -O3 -o FFT-fkp-mock-dr11-v7p0-wbossonly_960grid.exe FFT-fkp-mock-dr11-v7p0-wbossonly_960grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
do
    ...
    do
        ...
        then
            ...
            FFTname[$i]=$FFTdir$FFT$name0"00"$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        elif [ $i -lt 100 ]
        then
            fname=$datadir$name0"0"$i$nameend
            FFTname[$i]=$FFTdir$FFT$name0"0"$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        else
            fname=$datadir$name0$i$nameend
            FFTname[$i]=$FFTdir$FFT$name0$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        fi
        echo $fname
        echo ${FFTname[$i]}
        ./FFT-fkp-mock-dr11-v7p0-wbossonly_960grid.exe $Rbox 0 $P0 $datadir$nbarfname $fname ${FFTname[$i]}
    done    
    ...
    do
        if [ $i -lt 10 ]
        then
            powername=$powerdir$power$name0"00"$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        elif [ $i -lt 100 ]
        then
            powername=$powerdir$power$name0"0"$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        else
            powername=$powerdir$power$name0$i$nameend"_wbossonly.grid"$grid".P0"$P0".box"$box
        fi
