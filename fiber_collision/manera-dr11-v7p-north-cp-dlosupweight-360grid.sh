#!/bin/bash
name0="upweighted_cmass_dr11_north_ir4"
...
idl -e ".r fibcoll_pthalo_dlos_upweighted.pro"
ifort -O3 -o FFT-fkp-mock-dr11-v7p0-wboss.exe FFT-fkp-mock-dr11-v7p0-wboss.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
ifort -O3 -o FFT-fkp-mock-cp-dlosupweighted-360grid.exe FFT-fkp-mock-cp-dlosupweighted-360grid.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
do
    ...
    then
        ...
        FFTname=$FFTdir$FFT$name0"00"$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"00"$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
    elif [ $i -lt 100 ]
    then
        fname=$datadir$name0"0"$i$nameend
        FFTname=$FFTdir$FFT$name0"0"$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0"0"$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
    else
        fname=$datadir$name0$i$nameend
        FFTname=$FFTdir$FFT$name0$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
        powername=$powerdir$power$name0$i$nameend"-cp-dlosupweighted.grid"$grid".P0"$P0".box"$box
    fi
    ...
    then 
        ...
        idl -e "fibcoll_pthalo_dlos_upweighted,"$i",/north"
        echo $fname
    fi

    ./FFT-fkp-mock-cp-dlosupweighted-360grid.exe $Rbox 0 $P0 $datadir$nbarfname $fname $FFTname
    echo $FFTname
