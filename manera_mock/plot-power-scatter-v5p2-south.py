...
for j in range(len(P0)):
    for i in range(1,4):
        if i < 10:
            opt2name='power_cmass_dr10_south_ir400'+str(i)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'
        elif i < 100:
            opt2name='power_cmass_dr10_south_ir40'+str(i)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'
        else:
            opt2name='power_cmass_dr10_south_ir4'+str(i)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'

        opt2 = np.loadtxt(dir+opt2name)
        if i==1:
            opt2k = opt2[:,0]
            opt2tot = np.zeros([len(opt2[:,1])])
        opt2tot = opt2tot + opt2[:,1]
    opt2avg = opt2tot/float(i)

    for ii in range(1,4):
        if ii < 10:
            opt2name='power_cmass_dr10_south_ir400'+str(ii)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'
        elif ii < 100:
            opt2name='power_cmass_dr10_south_ir40'+str(ii)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'
        else:
            opt2name='power_cmass_dr10_south_ir4'+str(ii)+'.v5.2.wghtv.txt_wboss.grid360.P0'+str(P0[j])+'.box3600'
        opt2 = np.loadtxt(dir+opt2name)
        opt2diff = (opt2[:,1]-opt2avg)**2
#        [(opt2[x,1]-opt2avg[x])**2 for x in range(len(opt2))]
        if ii==1:
            opt2scat = np.zeros([len(opt2diff)])
        opt2scat = opt2scat + opt2diff
    opt2scat = np.sqrt(opt2scat/float(ii))/opt2avg

ax11.loglog(opt2k, opt2avg, color=colors[j], linewidth=2, label="P0="+str(P0[j])+" Option 2")
ax12.loglog(opt2k, opt2scat, color=colors[j],linewidth=2, label="P0="+str(P0[j])+" Option 2")
