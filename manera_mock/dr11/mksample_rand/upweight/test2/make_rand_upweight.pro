pro make_rand_upweight,n,zseed
    ...
    print, n_elements(rand_ra)
    vetomask_rand = where(rand_veto EQ 1 AND rand_w_boss GT 0 AND rand_w_red GT 0 AND rand_w_cp $
        GT 0)
    ...
    rand_az = fltarr(n_elements(rand_ra))
...
;            weights[i] = 1.0
    ...
    output_file = 'cmass_dr11_north_randoms_ir4'+strmid(strtrim(string(n+1000),1),1)+$
        '.v7.0.upweight.makerandom.test2.txt'
