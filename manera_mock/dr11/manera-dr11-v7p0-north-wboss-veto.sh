...
randname="cmass_dr11_north_"$n"_randoms_ir4_combined.v7.0.wboss.veto.txt"
FFTrandname=$FFTdir$FFT"cmass_dr11_north_"$n"_randoms_ir4_combined.v7.0.wboss.veto.grid"$grid".P0"$P0".box"$box
...
for i in $(seq 1 $n); do 
    ...
    FFTname=$FFTdir$FFT$name0$i".v7.0.wboss.veto.grid"$grid".P0"$P0".box"$box
    powername=$powerdir$power$name0$i".v7.0."$n"randoms.wboss.veto.grid"$grid".P0"$P0".box"$box
    echo $fname
