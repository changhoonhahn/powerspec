...
c      complex dcg(Ngrid/2+1,Ngrid,Ngrid),dcr(Ngrid/2+1,Ngrid,Ngrid)
      ...
      real akfun,kfun,I10,I12,I22,I13,I23,I33,P0,alpha,P0m
      real cot1,coga,Le2,Le4,pk,wsys,kmin,kmax,delk
      complex ct
      
      
      write(*,*) 'LSS/Mock Fourier file :'
      CALL GETARG(1,lssfft)
      write(*,*) 'Random Fourier file :'
      CALL GETARG(2,randfft)
      ...
      do 100 iz=1,Ngrid
         ...
         do 100 iy=1,Ngrid
            ...
            do 100 ix=1,Ngrid
               ...
               !Need to edit the imk in order to have the same k steps
               !as Florian's non-FKP method for fair comparison. 
               imk=nint(rk*akfun/kfun)
               if(imk.le.NbinsF .and. imk.ne.0)then
                  cot1=real(ikz)/rk
