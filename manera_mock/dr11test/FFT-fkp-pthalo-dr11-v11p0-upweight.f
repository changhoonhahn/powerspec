      ...
      integer rwb,rwcp,rwred,ripoly,rveto
      integer*8 planf
      ...
      real cz,sec2(Nsel),chi,nbar,Rbox,wsys,rwsys
...
!         write(*,*)'Random Survey File'
         ...
         rwsys=0.0
         do i=1,Nmax
            read(4,*,end=15)ra,dec,az,ripoly,rwb,rwcp,rwred,rveto
            ra=ra*(pi/180.)
            dec=dec*(pi/180.)
            rad=chi(az)
            IF (rwb.gt.0 .and. rwcp.gt.0 .and. rwred.gt.0 .and. 
     &      rveto.gt.0) THEN
                wr(i)=float(rwcp)+float(rwred)-1.0
            ELSE
                wr(i)=0.0
            ENDIF
            Nran=Nran+1
 ...
 15      continue
         ...
         WRITE(*,*) 'W_r,sys=',rwsys,'Ngal,sys=',rwsys/float(Nran)
