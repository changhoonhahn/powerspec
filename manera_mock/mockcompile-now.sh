...
ifort -O3 -o FFT-fkp-mock-now.exe FFT-fkp-mock-now.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
do 
    ...
    do
        ...
        echo ${fname[$i]}
        echo ${FFTname[$i]}
        ./FFT-fkp-mock-now.exe $Rbox 0 $P0 $nbarfname ${fname[$i]} ${FFTname[$i]}
    done
done 
