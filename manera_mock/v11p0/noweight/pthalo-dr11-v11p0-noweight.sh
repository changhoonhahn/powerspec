...
ifort -O3 -o FFT-fkp-pthalo-dr11-v11p0-noweight.exe FFT-fkp-pthalo-dr11-v11p0-noweight.f -L/usr/local/fftw_intel_s/lib -lsfftw -lsfftw
...
if [[ ! -a $FFTrandname ]]; then 
    echo $FFTrandname 
    ./FFT-fkp-pthalo-dr11-v11p0-noweight.exe $Rbox 1 $P0 $datadir$nbarfname $datadir$randname $FFTrandname
fi
...
for i in $(seq 1 $n); do 
    ...
    FFTname=$FFTdir$FFT$name0$i".v11.0.noweight.grid"$grid".P0"$P0".box"$box
    powername=$powerdir$power$name0$i".v11.0."$n"randoms.noweight.grid"$grid".P0"$P0".box"$box
    echo $fname
    ./FFT-fkp-pthalo-dr11-v11p0-noweight.exe $Rbox 0 $P0 $datadir$nbarfname $fname $FFTname
    echo $FFTname 
