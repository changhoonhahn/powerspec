...
do
    ...
    do
        ...
        then 
            ...
            powername[$i]=$power$name0"00"$i$nameend"_wboss_cp.grid"$grid".P0"$P0".box"$box
        elif [ $i -lt 100 ]
        then 
            FFTname[$i]=$FFT$name0"0"$i$nameend"_cp.grid"$grid".P0"$P0".box"$box
            powername[$i]=$power$name0"0"$i$nameend"_wboss_cp.grid"$grid".P0"$P0".box"$box
        else 
            FFTname[$i]=$FFT$name0$i$nameend"_cp.grid"$grid".P0"$P0".box"$box
            powername[$i]=$power$name0$i$nameend"_wboss_cp.grid"$grid".P0"$P0".box"$box
        fi 
